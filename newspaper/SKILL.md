...
description: 群日报/周报/月报 · 人民日报式 A3 报纸（印刷可用），版数可变（日报 2/4 版，周报 4/6 版，月报 6/8 版，一般偶数）。AI 先分析聊天记录 + 群内图片，根据报告类型和素材量决定版数，再为每版独立设计 layout：页眉页脚固定一致（左上角"第 N 版"按实际页码动态生成），中间内容由 AI 按当天具体情况决策（图嵌哪、跨几栏、配什么字、压缩什么）。严格 A3（297×420mm = 1123×1587px @96dpi），每版精确等高、零空白、零硬塞、不千篇一律、一页一版（page-break-inside: avoid + height: 1587px overflow hidden）。触发：报纸版、印刷版、纸质版、群报、做N版报纸、报纸版日报/周报/月报。
---
...
### 0. 版数按报告类型 + 当天故事量自适应（v2.0 升级）

**三步决定版数**：
1. 先看报告类型（日/周/月），确定基线版数范围
2. 再看 story.json 里 `timeline` 节点数，在范围内微调
3. 最终版数取偶数（印刷对折友好）

#### 日报（单天）

| timeline 节点数 | 版数 | 版面分配 |
|---|---|---|
| 2-3 节点 | **2 版** | page1（头版）+ page2（cast 模板：高光+SOP+附录） |
| 4-5 节点 | **4 版** | page1 + page2 共建 + page3 副刊 + page4 cast |
| 6+ 节点 | **4 版**（不扩到 6） | page1 + page2 共建 + page3 副刊 + page4 cast |
...
| 10+ 节点 | **6 版** | page1 + page2 共建 + page3 副刊 + page4 群体即兴 + page5 深夜专栏 + page6 cast |

#### 月报（~30 天）

| timeline 节点数 | 版数 | 版面分配 |
|---|---|---|
| 4-5 节点 | **6 版** | page1 + page2 共建 + page3 副刊 + page4 月度回顾 + page5 深度专栏 + page6 cast |
| 6-9 节点 | **8 版** | page1 + page2 共建 + page3 副刊 + page4 月度回顾 + page5 群体即兴 + page6 深度专栏 + page7 方法论 + page8 cast |
| 10+ 节点 | **8 版** | 同上，精选不堆砌 |
...
```bash
python3 newspaper/scripts/image_probe.py \
...
|---|---|---|
| 2-3 节点 | **2 版** | page1（头版）+ page2（cast 模板：高光+SOP+附录） |
| 4-6 节点 | **4 版** | page1 + page2 共建 + page3 副刊 + page4 cast |

	周报（7天）：
	| timeline 节点数 | 推荐版数 | 版面分配 |
	|---|---|---|
	| 4-5 节点 | **4 版** | page1 + page2 共建 + page3 副刊 + page4 cast |
	| 6-10 节点 | **6 版** | page1 + page2 共建 + page3 副刊 + page4 群体即兴 + page5 深度专栏 + page6 cast |

	月报（~30天）：
	| timeline 节点数 | 推荐版数 | 版面分配 |
	|---|---|---|
	| 4-5 节点 | **6 版** | page1 + page2 共建 + page3 副刊 + page4 回顾 + page5 方法论 + page6 cast |
	| 6-10 节点 | **8 版** | page1 + page2 共建 + page3 副刊 + page4 回顾 + page5 群体即兴 + page6 深度 + page7 方法论 + page8 cast |
...
```bash
python3 newspaper/scripts/render_newspaper.py \
  /tmp/story_<日期>_<群名>.json \
  /tmp/avatars.json \
  /tmp/layout-plan-<日期>-<群名>.json \
  ~/Desktop/<群名>报告_<日期>_报纸版.html
```
...
cat >> ~/Desktop/<群名>报告_<日期>_报纸版.html <<'EOF'
<script>
...
/Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome \
  ...
  file://~/Desktop/<群名>报告_<日期>_报纸版.html
...
## 为新群跑报告（v2.0 工作流摘要 · 日报/周报/月报通用）

```bash
# 1) group-daily skill 先产出 story.json + avatars.json
# 2) 识别报告类型（日/周/月），看 story.json timeline 节点数决定版数
	#    日报：2/4 版 | 周报：4/6 版 | 月报：6/8 版
# 3) 复制 plan 模板，按版数加 pageN
cp newspaper/examples/layout-plan-template.json \
   /tmp/layout-plan-<日期>-<群名>.json
