# vchat CLI · Mac 专属工具（Windows 版已弃用）

> **⚠️ 本文件是 macOS 原版的遗留参考文档。Windows 适配版已用 `wechat_windows.py` + PyWxDump 完全替代 vchat CLI。**
> 
> 如果你在 Windows 上使用本 skill，请忽略此文件。所有微信数据访问走 `scripts/wechat_windows.py`，详见 SKILL.md 的 8 步工作流。
> 
> 如果你在 macOS 上使用原版 group-daily，以下内容仍然有效。
